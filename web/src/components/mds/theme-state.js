...
/* ── Mật độ hiển thị: 'compact' | 'standard' (mặc định) | 'comfortable' ──
   Thuộc tính data-mds-space khớp đúng selector trong tokens/space-*.css
   (không phải "data-density" — bug đã gặp khi copy nguyên bản tham khảo). */
export const DENSITY_LIST = [
  { id: 'compact', label: 'Compact', height: 28, description: 'Thu gọn — hiển thị nhiều dữ liệu hơn' },
  { id: 'standard', label: 'Trung bình', height: 36, description: 'Mặc định — phù hợp đa số màn hình' },
  { id: 'comfortable', label: 'Rộng', height: 40, description: 'Rộng rãi — dễ thao tác trên màn to' },
]
const DENSITY_KEY = 'mds-density'
export const currentDensity = ref(readStorage(DENSITY_KEY, 'standard'))
export function applyDensity(id) {
  currentDensity.value = id
  if (id === 'standard') document.documentElement.removeAttribute('data-mds-space')
  else document.documentElement.setAttribute('data-mds-space', id)
  writeStorage(DENSITY_KEY, id)
}
if (currentDensity.value !== 'standard') document.documentElement.setAttribute('data-mds-space', currentDensity.value)
