...
- **Bản chính thức (production) — Google Cloud Run:** https://misa-event-checkin-784559735000.asia-southeast1.run.app
  ...
  - Deploy lại: `gcloud run deploy misa-event-checkin --source . --region asia-southeast1` (giữ nguyên env/scaling của service cũ). gcloud: máy Windows tại `%LOCALAPPDATA%\Google\Cloud SDK\...\gcloud.cmd`; **máy Mac (arm64) tại `~/google-cloud-sdk/bin/gcloud`** (đã cài + login `tuanbui88vn@gmail.com`).
...
- **Loại vị trí trong 1 sự kiện (`event_staff.staff_type`):** phân biệt hành vi của tài khoản `checkin` theo TỪNG sự kiện (1 người có thể là lễ tân sự kiện A, giám sát sự kiện B). Gán ở tab Nhân viên (chọn "Vai trò tại sự kiện" + "Vị trí").
  ...
  - `manager` (Quản lý xem số liệu) — không gán booth, KHÔNG bị khoá theo ngày (xem được số đăng ký trước/sau sự kiện). Chỉ có tab "📊 Số liệu": dashboard đếm đăng ký/đến dự + tỷ lệ %, lọc đa điều kiện (mức độ/chức vụ/quy mô/booth; trong nhóm = OR, giữa nhóm = AND) + bảng "tỷ trọng theo" 1 chiều (drill-down, VD booth+VIP rồi xem theo chức vụ để đếm CEO). Dùng endpoint `GET /events/:id/stats` trả về **vector thuộc tính ẩn danh** (không kèm tên/email/SĐT). `supervisor` + `manager` bị chặn `/report` + `/report/export` (có PII).
...
- **Phôi thẻ (badge):** bảng `badges` (mã số tuần tự/sự kiện, gán 1 khách, status active/stopped; 1 khách nhiều thẻ). Admin (tab Phôi thẻ) sinh phôi + xuất ZIP các SVG (QR mã hoá `{eventId}-{code}`) gửi nhà in; lễ tân/nhân viên (tab Gán thẻ, hiện khi `badge_count>0`) quét mã khách + mã phôi để gán + tự check-in; `scan` hiểu cả mã QR khách lẫn mã phôi (thêm status `badge_stopped`/`badge_unassigned`). In thẻ dự phòng `printQr()` khổ **100×75mm (A7)** - có QR + họ tên + chức danh + công ty + mức độ quan trọng (chốt 2026-07-28, theo mẫu chủ dự án cung cấp), qua trạm in (LAN/Agent) hoặc máy in USB nối trực tiếp máy đang mở web. Cần dep `jszip`. Chi tiết ở MEMORYBANK mục 5.7 và 27.
...
- 2026-07-01: thêm 3 loại vị trí **Lễ tân in QR** + **Giám sát booth** + **Quản lý (xem số liệu)** (xem mục Loại vị trí ở trên) — đã deploy Railway + Cloud Run + push GitHub.
