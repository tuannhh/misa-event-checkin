// Migration NỀN (baseline) - chuyển nguyên trạng schema đang chạy (trước đây tạo bằng
// các câu CREATE TABLE IF NOT EXISTS chạy tuần tự trong db.js) thành migration có version.
// Giữ nguyên IF NOT EXISTS để chạy an toàn trên DB đã có sẵn 11 bảng này (không tạo lại,
// không mất dữ liệu) lẫn DB hoàn toàn mới (tạo đủ từ đầu). Từ migration TIẾP THEO trở đi,
// dùng đúng kiểu knex.schema.alterTable(...)/createTable(...) chuẩn, không viết raw SQL nữa.
exports.up = async function up(knex) {
  ...
  for (const sql of statements) await knex.raw(sql);
};

exports.down = async function down() {
  throw new Error(
    'Migration baseline không hỗ trợ rollback tự động vì đây là dữ liệu sản xuất thật. ' +
    'Muốn xoá bảng thì tự chạy DROP TABLE thủ công, xác nhận kỹ trước khi làm.'
  );
};
